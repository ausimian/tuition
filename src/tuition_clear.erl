...
-moduledoc """
Clear widget — reset a rect to blanks, the overlay/popup primitive
(stateless).

Immediate-mode rendering composites widgets into one buffer in draw order
(`m:tuition_widget`), but nothing in that vocabulary *resets* a region:
...
an empty style: a `m:tuition_block` painting its background with the
...
its orphaned partner too (`m:tuition_render`), so no stray half is left
...
- `style` — the blank's style (default: unstyled, i.e. a plain
  default/default space). Set `bg` to lay a coloured backdrop under the
  overlay; leave it out to reset to the terminal default.
""".
...
-doc """
Reset every cell of `Area` to a blank space, returning the buffer with the
region wiped. A degenerate area (no columns or rows) draws nothing. Unlike
`tuition_widget:fill/3`, the default (empty) style still overwrites — the

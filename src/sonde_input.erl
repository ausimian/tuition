...
%%%
%%% The local backend's reader ({@link sonde_term_local}) forwards raw input
...
%%% with a bounded timeout ({@link sonde_term:read/2}); when the read times out
...
%%% capability probing ({@link sonde_caps}) has enabled the matching terminal mode:
%%% <ul>
...
%%% driver ({@link sonde_input_driver}), keeping decode logic separate from I/O.
%%% @end
%%%-------------------------------------------------------------------
-module(sonde_input).

...
%%% sonde_width}), not codepoints, so a line of CJK or emoji wraps and centres by
...
%%% sonde_widget:put_line/6}, which truncates it to the rect — so nothing ever
...
-spec render(paragraph(), #rect{}, sonde_render:buffer()) -> sonde_render:buffer().
...
draw_lines([Line | Rest], #rect{w = W} = Area, Align, Style, Row, Buf) ->
    %% Measure with the widget layer's sanitise-aware width, not sonde_width:swidth/1:
    ...
    Width = min(sonde_widget:display_width(Line), W),
    Col = sonde_widget:align_offset(Align, W, Width),
    Buf1 = sonde_widget:put_line(Buf, Area, Col, Row, Line, Style),
...
place(Word, W, Cur, Acc) ->
    case sonde_widget:display_width(Word) =< W of
...
place_fitting(Word, W, Cur, Acc) ->
    case sonde_widget:display_width(Cur) + 1 + sonde_widget:display_width(Word) =< W of
...
hard_split(Word, W, Acc) ->
    {Head, Rest} = sonde_widget:split(Word, W),
    case Rest of

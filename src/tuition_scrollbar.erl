%%%-------------------------------------------------------------------
%%% @doc Scrollbar widget — a track with a proportional thumb (stateless).
%%%
...
%%% is the ratatui `Scrollbar': the position indicator every scrollable pane in
%%% sonde (`sonde_procdetail', `sonde_apptree', the list/table views) wants beside
%%% its content, which today scrolls with no visual cue of extent.
%%%
%%% == Stateless ==
...
%%% immediate-mode rebuild. It implements the plain {@link tuition_widget} `render/3'
%%% callback.
%%%
%%% == Geometry ==
...
%%% <ul>
%%%   <li>`orientation' — `vertical' (default) | `horizontal'.</li>
%%%   <li>`content_length' — total items/lines being scrolled (default `0').</li>
%%%   <li>`viewport_length' — items/lines visible at once; defaults to the track
%%%       length (the area's long dimension, less any arrow caps).</li>
%%%   <li>`position' — the top offset of the window, clamped to the scrollable
%%%       range `[0, content_length - viewport_length]' (default `0').</li>
%%%   <li>`style' — style of the track (and the arrow caps); default unstyled.</li>
%%%   <li>`thumb_style' — style of the thumb; default unstyled.</li>
%%%   <li>`track' — the track glyph (default `│' vertical / `─' horizontal).</li>
%%%   <li>`thumb' — the thumb glyph (default `█').</li>
%%%   <li>`begin_symbol' / `end_symbol' — `none' (default) or an arrow-cap glyph at
...
%% @doc Draw the scrollbar into `Area'. A degenerate area (no columns or rows)

%%%-------------------------------------------------------------------
%%% @doc Scripted terminal backend for driving a render/input loop headlessly,
%%% with no tty. This is the framework's public test backend — ratatui's
%%% `TestBackend' plays the same role: point any host that opens a {@link
%%% sonde_term} backend at it ({@link sonde_shell:start/2}, {@link
%%% sonde_tui_demo:start/1}, or a pane test) and it replays a canned terminal so
...
%%%                list of sizes replayed one per {@link sonde_term:size/1} call
%%%                with the last sticking — so a test can drive a resize by
%%%                handing back a different size on a later poll.
%%%   * `script' — a list of {@link sonde_term:read/2} results replayed one per
...
%%% @end
%%%-------------------------------------------------------------------
-module(sonde_loop_term).
-behaviour(sonde_term).
...
-spec sizes(sonde_term:size() | [sonde_term:size()]) -> [sonde_term:size()].

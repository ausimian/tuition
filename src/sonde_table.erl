...
%%%
%%% == Layout ==
%%% Column widths are solved by the same {@link sonde_layout} constraint engine
%%% the panes are tiled with — each column carries a {@type
%%% sonde_layout:constraint()} (`{fixed, N}' / `{percent, P}' / `fill'), and the
...
%%% `include/sonde_widget.hrl') held by the *caller* — the renderer is
%%% immediate-mode and discards every frame, so state kept in the widget would
%%% not survive (see {@link sonde_widget}). The rows are exactly a {@link
%%% sonde_list} scrolled under the header, so {@link render/4} reuses {@link
%%% sonde_list:reconcile/3} to clamp the selection and slide the offset, and the
%%% navigation API ({@link next/2}, {@link prev/2}, {@link select/2}, {@link
%%% selected/1}) delegates to {@link sonde_list} — one source of truth for the
...
%%%       default `<<>>'), `constraint' (a {@type sonde_layout:constraint()},
...
-type column() :: #{
    ...
    constraint => sonde_layout:constraint(),
...
-type table_cfg() :: #{
    columns => [column()],
    rows => rows(),
    header_style => sonde_render:style(),
    row_style => sonde_render:style(),
    highlight_style => sonde_render:style(),
...
%% column wide in {@link sonde_width}, so {@link sonde_widget:display_width/1} and
...
-spec render(table_cfg(), #rect{}, sonde_render:buffer(), state()) ->
...
render(Cfg, #rect{w = W, h = H} = Area, Buf, State0) ->
    ...
    State1 = sonde_list:reconcile(State0, Len, Visible),
    Buf1 =
...
draw(Cfg, Columns, Rows, #rect{x = X, y = Y, w = W} = Area, HeaderRows, Visible, State, Buf) ->
    ...
    Gutter = sonde_widget:display_width(Symbol),
...
column_geometry(Columns, Spacing, ColsArea) ->
    ...
    Rects = sonde_layout:split(horizontal, Constraints, ColsArea),
...
-spec interleave([sonde_layout:constraint()], non_neg_integer()) ->
    [sonde_layout:constraint()].
interleave([], _Spacing) -> [];
...
draw_header(Buf, Geom, #rect{x = X, y = Y, w = W}, 1, Cfg) ->
    ...
    Buf1 = sonde_widget:fill(Buf, #rect{x = X, y = Y, w = W, h = 1}, Style),
...
%% row form, as cheap to *build* (see {@link sonde_list}).
-spec draw_rows(
    sonde_render:buffer(),
    [{non_neg_integer(), non_neg_integer(), column()}],
    rows(),
...
draw_row(Buf, Geom, Row, #rect{x = X, w = W}, RowY, Selected, Base, Highlight, Symbol) ->
    ...
    Buf1 = sonde_widget:fill(Buf, RowRect, Style),
    Buf2 =
        case Selected of
            true -> sonde_widget:put_line(Buf1, RowRect, 0, 0, Symbol, Style);
            false -> Buf1
        end,
    {BufN, _Idx} = lists:foldl(
        fun({Cx, Cw, Col}, {B, Idx}) ->
...
%% column and clipped to it (via {@link sonde_widget:put_line/6}, so it can never
...
draw_cell(Buf, Cx, Cw, Y, Text, Align, Style) ->
    ...
    Width = min(sonde_widget:display_width(Text), Cw),
    DCol = sonde_widget:align_offset(Align, Cw, Width),
    sonde_widget:put_line(Buf, CellRect, DCol, 0, Text, Style).

-module(tuition_tabs).
-moduledoc """
A horizontal row of titles with one selected (stateless).

A tab-bar draws a single row of titles separated by a divider glyph and
highlights the selected one, so a multi-pane UI can *show* the panes it switches
between and which one has focus. It is the ratatui `Tabs`. `m:tuition_shell`
already cycles panes with Tab, but nothing on screen names the available panes or
marks the focused one; a tab-bar makes that navigation visible, and its 0-based
`selected` index fits the shell's existing focus model.
...
caller passes it in as `selected` each frame. It implements the plain
`m:tuition_widget` `render/3` callback, with nothing threaded across the
immediate-mode rebuild.

## Layout

Titles are drawn along the top row of `Area`, left to right, each surrounded by
`padding` blank columns, with a `divider` glyph between adjacent titles:
...
widget a one-row strip (reserve it with `m:tuition_layout`, typically at the top
of a pane). A taller area is filled with the base `style`, but the titles sit on
its first row.
...
whole, never split), matching how `m:tuition_render` clips any run. Keep the
leading tabs visible by ordering them so the selected pane is not pushed off the
end, or size the strip to the titles.

## Config

A map, every key optional:

- `titles` — the tab titles, a list of chardata (default `[]`, an empty bar).
- `selected` — the 0-based index of the highlighted tab (default `0`). An index
  outside `[0, length(titles))` highlights nothing, so an empty bar or a stale
  index simply draws every title in the base style.
- `style` — the base style for the whole bar: the background fill and the style
  of the unselected titles and the dividers (default: unstyled, so the bar is
  transparent over whatever it is drawn onto).
- `highlight_style` — the selected title's style, overlaid on `style` (so it
  need only name the keys that differ); default unstyled, i.e. the selected
  title looks like the rest until this is set.
- `divider` — the glyph drawn between adjacent titles (default `│`).
- `padding` — columns of space on each side of every title (default `1`, clamped
  at `0`). Like the rest of the bar, these follow `style`: painted with the base
  fill when it is set, and left transparent — showing the parent background, or
  whatever the bar is drawn over — when it is not. They are never overwritten
  with blank cells, so an unstyled bar composes cleanly over a parent block's
  coloured strip, matching `tuition_widget:fill/3`.
- `title_align` — `left` (default), `center` or `right`, where the strip sits
  within `Area` when it is narrower than the area.

Only the selected title's glyphs carry `highlight_style`; the padding around it
keeps the base style, matching ratatui's per-title highlight.
""".
...
-doc """
Draw the tab-bar into `Area`. An empty area (no columns or rows) draws nothing.

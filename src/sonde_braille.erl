...
%%%
%%% A terminal cell is the smallest thing the {@link sonde_render} buffer can
...
%%% glyphs a {@link sonde_sparkline} draws with. This module is that grid — the
%%% genuinely reusable kernel {@link sonde_chart} rasterizes trend curves onto,
...
%%%
%%% == Not a widget ==
%%% Like {@link sonde_width}, this is an internal shared helper, not a {@link
%%% sonde_widget}: it holds no config map and implements no `render/3'. A widget
%%% ({@link sonde_chart}) owns a grid for the duration of one frame, plots onto it,
...
%%% flips the axis itself (as {@link sonde_chart} does). A sub-pixel outside
...
%%% sonde_chart} documents for overlapping datasets. {@link set/3} carries no
...
%%% Every braille glyph is one column in {@link sonde_width}, so the grid
%%% composites through {@link sonde_render:diff/2} exactly as any other width-1
%%% text — no special-casing in the renderer.
%%%
%%% HARD CONSTRAINT (PRD §12): depends only on `kernel'/`stdlib'/`erts' plus the
%%% sibling {@link sonde_render} buffer. No third-party code.
%%% @end
%%%-------------------------------------------------------------------
-module(sonde_braille).
...
%% half of {@link sonde_render:style()}; `default' leaves the cell's foreground
...
%% cell exactly as {@link sonde_chart} documents, whatever colour it carries. A
...
%% sonde_chart} connects consecutive samples into a continuous curve. Endpoints
...
-spec render_into(grid(), sonde_render:buffer()) -> sonde_render:buffer().
...
-spec render_into(grid(), sonde_render:buffer(), sonde_render:style()) -> sonde_render:buffer().
render_into(#grid{rect = #rect{x = X, y = Y}, cells = Cells}, Buf, Style) ->
    maps:fold(
        fun({CX, CY}, {Mask, Colour}, B) ->
            Glyph = ?BRAILLE_BASE bor Mask,
            sonde_render:put_text(B, X + CX, Y + CY, <<Glyph/utf8>>, cell_style(Style, Colour))
        end,
        Buf,
        Cells
    ).
...
%% any, stands).
-spec cell_style(sonde_render:style(), colour()) -> sonde_render:style().

...
%%% ```
%%%   Prev0 = sonde_render:new(Size),                 %% blank screen
%%%   Next  = build_frame(State),                     %% put_text/put_cell ...
%%%   ok    = sonde_term:write(Handle, sonde_render:diff(Prev0, Next)),
...
%%% sibling `sonde_width'/`sonde_layout' modules). No third-party code.
%%% @end
%%%-------------------------------------------------------------------
-module(sonde_render).
...
-spec new(sonde_term:size() | #rect{}) -> buffer().
...
%%   * a malformed cluster wider than two columns (sonde_width deliberately sums
...
sanitize_glyph(GC) ->
    ...
    case sonde_width:width(Safe) > 2 of
        true -> ?REPLACEMENT;
        false -> Safe
    end.

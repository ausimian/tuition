...
%%% tuition_term} backend at it ({@link tuition_shell:start/2}, {@link
%%% tuition_demo:start/1}, or a pane test) and it replays a canned terminal so
...
%%%                list of sizes replayed one per {@link tuition_term:size/1} call
%%%                with the last sticking — so a test can drive a resize by
%%%                handing back a different size on a later poll.
%%%   * `script' — a list of {@link tuition_term:read/2} results replayed one per

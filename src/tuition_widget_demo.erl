...
%%% tuition_block} frames the pane, {@link tuition_paragraph} draws a help line, and a
%%% stateful {@link tuition_table} shows a scrollable, selectable, sortable table of
...
%%% real {@link tuition_render} diff renderer, {@link tuition_layout} split, and an
%%% immediate-mode input loop — the same shape as {@link tuition_demo}'s Phase 0
%%% loop, so the two sit side by side rather than one replacing the other.
%%%
%%% == Running it ==
%%% `tuition_widget_demo:start()' runs the demo on its own, hosted by {@link
%%% tuition_shell}, until `q' (or Ctrl-C). Up/Down (or `j'/`k') move the selection; the
...
%%% table widget (see {@link tuition_widget}). Input folds into the selection and
...
%% @doc Run the demo on its own, hosted by {@link tuition_shell}. Blocks until the
...
%% (`backend' selects it, default {@link tuition_term_local}; the whole `Opts' map is
...
%% refresh. Present to satisfy the {@link tuition_pane} contract the shell drives
...
%% {@link tuition_block} framing a {@link tuition_paragraph} help line above a stateful,
%% sortable {@link tuition_table}. The rows are ordered by the current sort for

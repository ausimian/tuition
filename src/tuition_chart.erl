...
%%% labels are the caller's to compose (a {@link tuition_block} title, an adjacent
%%% {@link tuition_paragraph}), as {@link tuition_sparkline} leaves them, since their
...
%% tuition_widget:put_line/6}, so a one-column or one-row area degrades to just the
%% part of the frame that fits (the plot rect then coming back degenerate, drawn
%% as nothing by {@link render/3}).
-spec draw_axes(tuition_render:buffer(), #rect{}, tuition_render:style()) ->
    {#rect{}, tuition_render:buffer()}.
draw_axes(Buf, #rect{x = X, y = Y, w = W, h = H} = Area, Style) ->
    Bottom = H - 1,
    %% y-axis: the left column, every row above the corner.
    Buf1 = lists:foldl(
        fun(Row, B) -> tuition_widget:put_line(B, Area, 0, Row, <<?V_AXIS/utf8>>, Style) end,
        Buf,
        lists:seq(0, Bottom - 1)
    ),
    %% x-axis: the bottom row, every column right of the corner.
    Buf2 = lists:foldl(
        fun(Col, B) -> tuition_widget:put_line(B, Area, Col, Bottom, <<?H_AXIS/utf8>>, Style) end,
        Buf1,
        lists:seq(1, W - 1)
    ),
    Buf3 = tuition_widget:put_line(Buf2, Area, 0, Bottom, <<?CORNER/utf8>>, Style),

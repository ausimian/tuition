...
%%% sibling {@link sonde_term} seam). No third-party code.
%%% @end
%%%-------------------------------------------------------------------
-module(sonde_caps).
...
-spec probe(sonde_term:handle()) -> {caps(), binary()}.
...
-spec probe(sonde_term:handle(), timeout()) -> {caps(), binary()}.
probe(Handle, Timeout) ->
    case sonde_term:write(Handle, ?QUERIES) of
...
-spec read_until_sentinel(sonde_term:handle(), timeout(), binary()) -> binary().

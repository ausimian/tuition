...
%%%
%%% {@link sonde_input} is a pure, timerless decoder; this module supplies the
...
%%% itself lives in {@link sonde_term:read/2}; {@link poll/3} connects a read
%%% `timeout' to {@link sonde_input:flush/1}.
%%%
...
%%% ```
%%%   loop(Handle, St0) ->
%%%       case sonde_input_driver:poll(Handle, St0, 1000) of
%%%           {ok, Events, St1} ->
...
%% buffered ({@link sonde_input:awaiting_escape/1}) so a lone `ESC' resolves to
...
%%
%% Incoming bytes are decoded via {@link sonde_input:parse/2}. A read `timeout'
%% flushes the buffered partial via {@link sonde_input:flush/1} ONLY when it is
...
-spec poll(sonde_term:handle(), sonde_input:state(), timeout()) ->
    {ok, [sonde_input:event()], sonde_input:state()} | {error, term()}.
poll(Handle, St, IdleTimeout) ->
    AwaitingEsc = sonde_input:awaiting_escape(St),
    Timeout =
        case AwaitingEsc of
            true -> ?ESC_TIMEOUT;
            false -> IdleTimeout
        end,
    case sonde_term:read(Handle, Timeout) of
        {ok, Bytes} ->
            {Events, St1} = sonde_input:parse(Bytes, St),
            {ok, Events, St1};
        timeout when AwaitingEsc ->
            {Events, St1} = sonde_input:flush(St),
            {ok, Events, St1};
        timeout ->

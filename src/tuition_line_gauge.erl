...
-moduledoc """
A single-row progress indicator (stateless).

A line gauge draws a label and then a thin horizontal line across the rest of one
row: the leading fraction of the line drawn `filled`, the remainder `unfilled`,
so a `ratio` in `[0, 1]` reads at a glance in a single cell of height. It is the
ratatui `LineGauge`, the compact sibling of `m:tuition_gauge`. Reach for it in
dense dashboards where the full-height bar is too heavy and you want many metrics
stacked one per line rather than a few thick bars.

## vs. the full gauge

`m:tuition_gauge` fills its whole area with a solid block bar (a taller area is a
thicker bar) and centres the label over it. A line gauge instead commits to one
row: label at the left, a one-cell-high line filling the width to its right. Give
it a one-row rect (the common tile); a taller rect is drawn on its top row and
the rows below are left untouched, so several line gauges tile compactly down a
column.
...
partial-fill form, and the fraction shows through the styling split rather than a
boundary glyph. (The full `m:tuition_gauge` keeps its eighth-block sub-cell
precision; this widget trades it for the single-row footprint.)

## Stateless

A line gauge holds no state between frames: the caller recomputes its `ratio`
each frame from whatever it is measuring and passes it in as config, exactly as
for `m:tuition_gauge`. It implements the plain `m:tuition_widget` `render/3`
...
- `ratio` — the fill fraction, a number in `[0.0, 1.0]`. Values outside that
  range are clamped, so a measurement glitch can never draw a line past the area
  or a negative length. Default `0.0`.
- `label` — `none` for no label (the line then spans the full width), or chardata
  to draw instead of the default. Omit it and the label is the rounded percentage
  (e.g. `"63%"`). The line begins one blank column after the label.
- `line` — the line glyph: `thin` (default, `─` U+2500) or `heavy` (`━` U+2501),
  the ratatui light/thick pair, or custom single-cell chardata used for the whole
  rule.
- `filled_style` — the style of the filled leading run (default: unstyled, a
  default-foreground line; set at least `fg` to colour it).
- `unfilled_style` — the style of the unfilled trailing run (default: unstyled).
...
`filled_style` and `unfilled_style` differ (a colour, or bold). This matches
...
-doc """
Draw the line gauge into `Area`, on its top row. An empty area (no columns or
rows) draws nothing. See the module doc for the config map.
""".

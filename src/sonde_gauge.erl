%%%-------------------------------------------------------------------
%%% @doc Gauge widget — a horizontal progress bar with sub-cell precision.
%%%
...
%%% dashboard primitive the system-dashboard tiles (PRD §9.1) show a memory
...
%%% {@link sonde_width}, so the bar's width and the renderer's cursor advance
%%% agree.
%%%
%%% == Stateless ==
...
%%% time delta) and passed in as config. It implements the plain {@link
%%% sonde_widget} `render/3' callback — nothing to thread across the immediate-mode
%%% rebuild.
%%%
%%% == Config ==
...
%%%       area). Default `0.0'.</li>
%%%   <li>`label' — `none' to draw no label, or chardata to draw instead of the
%%%       default. Absent, the label is the rounded percentage (e.g. `"63%"').</li>
%%%   <li>`label_align' — `left' | `center' (default) | `right', where the label
%%%       sits within the area.</li>
%%%   <li>`fill_style' — the style of the filled bar glyphs (default: unstyled — a
%%%       default-foreground bar; set at least `fg' to colour it).</li>
%%%   <li>`unfilled_style' — a style for the track behind the unfilled remainder
%%%       (default: unstyled — the track is transparent, showing whatever the
%%%       gauge is drawn over).</li>
%%%   <li>`label_style' — the label's style (default: unstyled — drawn in the
...
-type gauge() :: #{
    ...
    fill_style => sonde_render:style(),
    unfilled_style => sonde_render:style(),
    label_style => sonde_render:style()
}.
...
%% @doc Draw the gauge into `Area'. A degenerate area (no columns or rows) draws
%% nothing. See the module doc for the config map.
-spec render(gauge(), #rect{}, sonde_render:buffer()) -> sonde_render:buffer().
...
render(Gauge, #rect{w = W} = Area, Buf0) ->
    ...
    Buf1 = sonde_widget:fill(Buf0, Area, UnfilledStyle),
...
-spec draw_bars(
    sonde_render:buffer(), #rect{}, non_neg_integer(), 0..8, sonde_render:style()
) -> sonde_render:buffer().
...
%% a column is left for it). {@link sonde_widget:put_line/6} clips both to the
...
draw_bar_row(Buf, Area, Row, Full, Eighths, Style) ->
    Buf1 =
        case Full of
            0 -> Buf;
            _ -> sonde_widget:put_line(Buf, Area, 0, Row, binary:copy(<<?FULL/utf8>>, Full), Style)
        end,
    case Eighths of
        0 -> Buf1;
        _ -> sonde_widget:put_line(Buf1, Area, Full, Row, <<(horiz_block(Eighths))/utf8>>, Style)
    end.
...
%% through the bar.
-spec draw_label(sonde_render:buffer(), #rect{}, gauge(), float()) -> sonde_render:buffer().
draw_label(Buf, #rect{w = W, h = H} = Area, Gauge, Ratio) ->
    case label_text(Gauge, Ratio) of
        ...
        Text ->
            ...
            Width = min(sonde_widget:display_width(Text), W),
            Col = sonde_widget:align_offset(Align, W, Width),
            sonde_widget:put_line(Buf, Area, Col, H div 2, Text, Style)
    end.

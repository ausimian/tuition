-module(tuition_pane).
-moduledoc """
Pane behaviour — the contract the app shell hosts.

Every pane the shell hosts is a module implementing this behaviour, so `m:tuition_shell` can host any of them without knowing which: it drives the shared
...
This is the seam between the shell chrome and the panes it
...
A pane is a pure state machine: `new/0` seeds it, `apply_events/2`
folds input into it, `sample/1` refreshes it from the live node, and
`render/3` draws it. The shell owns each pane's state and threads it back
across frames — the renderer is immediate-mode (see `m:tuition_widget`), so a
...
- `render/3` — draw the pane into the rect the shell allocated it (the
  ...
  all stateless returns its state unchanged.
- `apply_events/2` — fold a batch of input events into the state,
  ...
  unmodified `q` in most panes — but *not*, say, while a text field
  ...
  changed *what the visible panes observe* — the node picker switching
  the active target — returns `{sample, State}` instead of `{ok, State}` to
  ask the shell to run its `sample/1` cycle right after this batch,
  ...
  `{ok, State}`.
- `sample/1` — refresh the state from the running node. Impure. The
  shell calls it on the idle tick for the focused pane only; a static pane
  (no live data) returns its state unchanged.
- `setup/0` / `teardown/1` — optional lifecycle hooks for a pane
  ...
  that needs neither omits both.

## Global vs. pane-local input

The shell routes keys: it owns the *global* keys (Tab/Shift-Tab to switch
panes, Ctrl-C to quit) and passes everything else to the focused pane's `apply_events/2`. The global keys are deliberately ones no pane binds — Tab is

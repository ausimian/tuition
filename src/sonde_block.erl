...
%%% rect with {@link sonde_layout} and frame each child in its own block.
%%%
%%% == Config ==
...
-type block() :: #{
    ...
    title_style => sonde_render:style(),
    border_style => sonde_render:style(),
...
-spec render(block(), #rect{}, sonde_render:buffer()) -> sonde_render:buffer().
...
-spec fill_background(block(), #rect{}, sonde_render:buffer()) -> sonde_render:buffer().
fill_background(Block, Area, Buf) ->
    case maps:find(style, Block) of
        error -> Buf;
        {ok, Style} -> sonde_widget:fill(Buf, Area, Style)
    end.
...
-spec draw_border([side()], #rect{}, sonde_render:style(), sonde_render:buffer()) ->
    sonde_render:buffer().
...
-spec edge(boolean(), fun((sonde_render:buffer()) -> sonde_render:buffer()), sonde_render:buffer()) ->
    sonde_render:buffer().
...
-spec hline(sonde_render:buffer(), integer(), integer(), non_neg_integer(), sonde_render:style()) ->
    sonde_render:buffer().
hline(Buf, X, Y, W, Style) ->
    sonde_render:put_text(Buf, X, Y, binary:copy(<<?HORIZ/utf8>>, W), Style).

%% A vertical run of H box-drawing bars down column X, one cell per row.
-spec vline(sonde_render:buffer(), integer(), integer(), non_neg_integer(), sonde_render:style()) ->
    sonde_render:buffer().
...
-spec draw_title(block(), [side()], #rect{}, sonde_render:buffer()) -> sonde_render:buffer().
        ...
        {ok, Title} ->
            ...
            case Avail =< 0 of
                true ->
                    Buf;
                false ->
                    Clipped = sonde_widget:truncate(Title, Avail),
                    Align = maps:get(title_align, Block, left),
                    Pad = sonde_widget:align_offset(
                        Align, Avail, sonde_widget:display_width(Clipped)
                    ),
                    Style = maps:get(title_style, Block, border_style(Block)),
                    sonde_render:put_text(Buf, X + L + Pad, Y, Clipped, Style)
            end
    end.
...
-spec border_style(block()) -> sonde_render:style().
border_style(Block) -> maps:get(border_style, Block, #{}).

-spec put_glyph(sonde_render:buffer(), integer(), integer(), char(), sonde_render:style()) ->
    sonde_render:buffer().
put_glyph(Buf, X, Y, Glyph, Style) ->
    sonde_render:put_text(Buf, X, Y, <<Glyph/utf8>>, Style).

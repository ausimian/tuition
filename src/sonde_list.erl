...
%%% `include/sonde_widget.hrl') held by the *caller*, not in this module — the
%%% renderer is immediate-mode and discards every frame, so state kept inside the
%%% widget would not survive (see {@link sonde_widget}). {@link render/4} takes the
...
%%% ```
%%%   State1 = sonde_list:next(State0, length(Items)),   %% on Down
%%%   {Buf1, State2} = sonde_list:render(Cfg, Area, Buf0, State1).
%%% '''
...
%% to include `sonde_widget.hrl' can start here and drive it through the API
...
-spec render(list_cfg(), #rect{}, sonde_render:buffer(), state()) ->
...
%% exported so {@link sonde_table} — whose rows are a `#list_state{}' scrolled
...
draw_items(Cfg, Items, #rect{h = H} = Area, #list_state{selected = Selected, offset = Offset}, Buf) ->
    ...
    Gutter = binary:copy(<<" ">>, sonde_widget:display_width(Symbol)),
...
draw_row(Buf, Area, Row, Selected, Item, Base, Highlight, Symbol, Gutter) ->
    ...
    Buf1 = sonde_widget:fill(Buf, row_rect(Area, Row), Style),
    sonde_widget:put_line(Buf1, Area, 0, Row, [Prefix, Item], Style).

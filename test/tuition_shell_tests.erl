...
%%% Two stub panes, in tab order (see tuition_shell_pane_a/_b) — distinguishable content.
-define(PANES, [{tuition_shell_pane_a, <<"Alpha">>}, {tuition_shell_pane_b, <<"Beta">>}]).
...
frame(Shell) ->
    {Buf, _Shell} = tuition_shell:build_frame(?BIG, Shell),
    iolist_to_binary(tuition_render:diff(tuition_render:new(?BIG), Buf)).

%%% -- construction ----------------------------------------------------

new_focuses_the_first_pane_test() ->
    Shell = tuition_shell:new(?PANES),
    ?assertEqual(0, tuition_shell:active(Shell)),
    ?assertEqual(tuition_shell_pane_a, tuition_shell:active_module(Shell)),
    ?assertEqual(2, tuition_shell:pane_count(Shell)).

new_can_focus_a_later_pane_test() ->
    Shell = tuition_shell:new(?PANES, 1),
    ?assertEqual(1, tuition_shell:active(Shell)),
    ?assertEqual(tuition_shell_pane_b, tuition_shell:active_module(Shell)).
...
tab_switches_to_the_next_pane_test() ->
    {ok, S1} = tuition_shell:apply_events([tab()], tuition_shell:new(?PANES)),
    ?assertEqual(1, tuition_shell:active(S1)),
    ?assertEqual(tuition_shell_pane_b, tuition_shell:active_module(S1)).

shift_tab_switches_to_the_previous_pane_test() ->
    {ok, S1} = tuition_shell:apply_events([backtab()], tuition_shell:new(?PANES, 1)),
    ?assertEqual(0, tuition_shell:active(S1)).

tab_wraps_around_the_ends_test() ->
    %% Tab off the last pane returns to the first; Shift-Tab off the first to the last.
    {ok, Wrapped} = tuition_shell:apply_events([tab()], tuition_shell:new(?PANES, 1)),
    ?assertEqual(0, tuition_shell:active(Wrapped)),
    {ok, Back} = tuition_shell:apply_events([backtab()], tuition_shell:new(?PANES, 0)),
    ?assertEqual(1, tuition_shell:active(Back)).

switching_shows_the_newly_focused_pane_test() ->
    %% The nav tabs are always present; the body shows whichever pane has focus.
    S0 = tuition_shell:new(?PANES),
    B0 = frame(S0),
...
single_pane_shell_draws_no_nav_bar_test() ->
    ...
    Shell = tuition_shell:new([{tuition_shell_pane_b, <<"Beta">>}]),
    ?assertEqual(1, tuition_shell:pane_count(Shell)),
    B = frame(Shell),
...
pane_local_keys_reach_the_focused_pane_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    ?assertEqual(0, tuition_shell_pane_b:selection(tuition_shell:active_state(S0))),
    {ok, S1} = tuition_shell:apply_events([{key, down, []}, char($j)], S0),
    ?assertEqual(2, tuition_shell_pane_b:selection(tuition_shell:active_state(S1))).
...
switch_mid_batch_retargets_later_events_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    {ok, S1} = tuition_shell:apply_events([{key, down, []}, tab(), {key, down, []}], S0),
    ?assertEqual(0, tuition_shell:active(S1)),
    %% Switching back shows pane B advanced exactly once (row 1).
    {ok, S2} = tuition_shell:apply_events([backtab()], S1),
    ?assertEqual(1, tuition_shell_pane_b:selection(tuition_shell:active_state(S2))).
...
tab_switches_even_while_a_pane_captures_text_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    {ok, Filtering} = tuition_shell:apply_events([char($/)], S0),
    {ok, Switched} = tuition_shell:apply_events([tab()], Filtering),
    ?assertEqual(0, tuition_shell:active(Switched)).
...
q_is_pane_local_and_types_into_an_active_filter_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    Result = tuition_shell:apply_events([char($/), char($q)], S0),
...
ctrl_c_quits_from_any_pane_test() ->
    ?assertEqual(quit, tuition_shell:apply_events([ctrl_c()], tuition_shell:new(?PANES, 0))),
    ?assertEqual(quit, tuition_shell:apply_events([ctrl_c()], tuition_shell:new(?PANES, 1))).
...
ctrl_c_quits_even_while_a_pane_captures_text_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    {ok, Filtering} = tuition_shell:apply_events([char($/)], S0),
    ?assertEqual(quit, tuition_shell:apply_events([ctrl_c()], Filtering)).
...
q_quits_when_the_focused_pane_treats_it_as_quit_test() ->
    ...
    ?assertEqual(quit, tuition_shell:apply_events([char($q)], tuition_shell:new(?PANES, 0))),
    ?assertEqual(quit, tuition_shell:apply_events([char($q)], tuition_shell:new(?PANES, 1))).

%%% -- chrome ----------------------------------------------------------

status_line_names_the_global_keys_test() ->
    B = frame(tuition_shell:new(?PANES)),
...
tiny_terminal_does_not_crash_test() ->
    ...
    Shell = tuition_shell:new(?PANES),
    ?assertMatch({_, _}, tuition_shell:build_frame({4, 3}, Shell)),
    ?assertMatch({_, _}, tuition_shell:build_frame({1, 1}, Shell)),
    ?assertMatch({_, _}, tuition_shell:build_frame({20, 2}, Shell)).
...
sample_refreshes_only_the_focused_pane_test() ->
    ...
    S0 = tuition_shell:new(?PANES, 1),
    S1 = tuition_shell:sample(S0),
    ?assert(length(tuition_shell_pane_b:rows(tuition_shell:active_state(S1))) > 0),
...
is_idle_tick_only_on_a_genuine_timeout_test() ->
    ...
    {[], MidEsc} = tuition_input:parse(<<16#1B>>, Clean),
    ?assertNot(tuition_shell:is_idle_tick([], MidEsc)),
    {[], MidCsi} = tuition_input:parse(<<16#1B, $[>>, Clean),
    ?assertNot(tuition_shell:is_idle_tick([], MidCsi)),
    %% A completed keystroke is never an idle tick.
    ?assertNot(tuition_shell:is_idle_tick([{key, up, []}], Clean)).
...
%%% (tuition_shell_order_pane) that logs each `setup/0'/`teardown/1' to the process
...
lifecycle_opts() ->
    #{backend => tuition_loop_term, sink => self(), size => ?BIG, script => [{ok, <<3>>}]}.
...
reset_lifecycle() ->
    erase(tuition_pane_lifecycle),
    erase(tuition_pane_lifecycle_seq),
    ok.
...
teardown_unwinds_in_reverse_acquisition_order_test() ->
    ...
    reset_lifecycle(),
    Panes = [{tuition_shell_order_pane, <<"A">>}, {tuition_shell_order_pane, <<"B">>}],
...
setup_crash_restores_terminal_and_earlier_resources_test() ->
    ...
    reset_lifecycle(),
    Panes = [{tuition_shell_order_pane, <<"A">>}, {tuition_shell_boom_pane, <<"boom">>}],
    ?assertError(boom, tuition_shell:start(Panes, lifecycle_opts())),
...
opts_focus_a_pane_by_module_test() ->
    ...
    Opts = #{
        ...
        size => ?BIG,
        script => [{ok, <<"q">>}],
        active => tuition_shell_pane_b
    },
    ?assertEqual(ok, tuition_shell:start(?PANES, Opts)),
...
%%% -- sidebar ---------------------------------------------------------
%%%
%%% The stub sidebar pane (tuition_shell_side_pane) draws "side body", counts its
...
-define(SIDEBAR, #{module => tuition_shell_side_pane, title => <<"Side">>, width => 20}).

sidebar_shell() -> sidebar_shell(0).
sidebar_shell(Active) -> tuition_shell:new(?PANES, Active, #{sidebar => ?SIDEBAR}).

focus_at(Shell) -> {tuition_shell:focus(Shell), tuition_shell:active(Shell)}.
...
sidebar_starts_focused_on_the_sidebar_test() ->
    ...
    S = sidebar_shell(),
    ?assertEqual(sidebar, tuition_shell:focus(S)),
    ?assertEqual(0, tuition_shell:active(S)),
    ?assertEqual(tuition_shell_side_pane, tuition_shell:focused_module(S)).
...
tab_ring_includes_the_sidebar_as_the_leftmost_stop_test() ->
    ...
    S0 = sidebar_shell(),
    ?assertEqual(sidebar, tuition_shell:focus(S0)),
    {ok, S1} = tuition_shell:apply_events([tab()], S0),
    ?assertEqual({main, 0}, focus_at(S1)),
    {ok, S2} = tuition_shell:apply_events([tab()], S1),
    ?assertEqual({main, 1}, focus_at(S2)),
    {ok, S3} = tuition_shell:apply_events([tab()], S2),
    ?assertEqual(sidebar, tuition_shell:focus(S3)).
...
shift_tab_from_the_sidebar_wraps_to_the_last_main_pane_test() ->
    ...
    %% last main pane.
    {ok, S1} = tuition_shell:apply_events([backtab()], sidebar_shell()),
...
sample_refreshes_the_sidebar_and_the_active_pane_test() ->
    ...
    S1 = tuition_shell:sample(sidebar_shell()),
    ?assertEqual(1, tuition_shell_side_pane:samples(tuition_shell:sidebar_state(S1))),
    ?assert(length(tuition_shell_pane_a:rows(tuition_shell:active_state(S1))) > 0).
...
keys_route_to_the_sidebar_while_it_has_focus_test() ->
    ...
    {ok, S2} = tuition_shell:apply_events([{key, down, []}], sidebar_shell()),
    ?assertEqual(1, tuition_shell_side_pane:moved(tuition_shell:sidebar_state(S2))),
    ?assertEqual(0, tuition_shell_pane_a:selection(tuition_shell:active_state(S2))).
...
keys_route_to_the_main_pane_while_it_has_focus_test() ->
    ...
    {ok, S0} = tuition_shell:apply_events([tab()], sidebar_shell()),
    ?assertEqual({main, 0}, focus_at(S0)),
    {ok, S1} = tuition_shell:apply_events([{key, down, []}], S0),
    ?assertEqual(1, tuition_shell_pane_a:selection(tuition_shell:active_state(S1))),
    ?assertEqual(0, tuition_shell_side_pane:moved(tuition_shell:sidebar_state(S1))).

status_right_is_shown_on_the_status_line_test() ->
    S = tuition_shell:new(?PANES, 0, #{
...
sidebar_tiny_terminal_does_not_crash_test() ->
    ...
    ?assertMatch({_, _}, tuition_shell:build_frame({4, 3}, S)),
    ?assertMatch({_, _}, tuition_shell:build_frame({1, 1}, S)),
    ?assertMatch({_, _}, tuition_shell:build_frame({20, 2}, S)).
...
sidebar_sample_request_forces_a_sample_off_a_keystroke_test() ->
    ...
    ?assertEqual(sidebar, tuition_shell:focus(OnSidebar)),
    %% A non-switch key: no forced sample (a keystroke, and no request armed).
    {ok, Nav} = tuition_shell:apply_events([{key, down, []}], OnSidebar),
    Nav1 = tuition_shell:maybe_sample([{key, down, []}], tuition_input:new(), Nav),
    ?assertEqual([], tuition_shell_pane_a:rows(tuition_shell:active_state(Nav1))),
    %% Enter arms the request: the active main pane samples despite the keystroke.
    {ok, Switched} = tuition_shell:apply_events([{key, enter, []}], OnSidebar),
    Switched1 = tuition_shell:maybe_sample([{key, enter, []}], tuition_input:new(), Switched),
    ?assert(length(tuition_shell_pane_a:rows(tuition_shell:active_state(Switched1))) > 0).
...
sidebar_is_set_up_and_torn_down_with_the_main_panes_test() ->
    ...
    reset_lifecycle(),
    Panes = [{tuition_shell_order_pane, <<"A">>}],
    Opts = (lifecycle_opts())#{
        sidebar => #{module => tuition_shell_order_pane, title => <<"Side">>, width => 10}
    },
    ?assertEqual(ok, tuition_shell:start(Panes, Opts)),
    drain(<<>>, false),
    ?assertEqual([{setup, 1}, {setup, 2}, {teardown, 2}, {teardown, 1}], lifecycle_log()).

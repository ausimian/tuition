...
render(Cfg, W, H, State) ->
    sonde_list:render(Cfg, rect(0, 0, W, H), buf(W, H), State).

%%% -- navigation (pure) -----------------------------------------------

next_from_none_selects_first_test() ->
    ?assertEqual(0, sonde_list:selected(sonde_list:next(sonde_list:new(), 5))).

prev_from_none_selects_last_test() ->
    ?assertEqual(4, sonde_list:selected(sonde_list:prev(sonde_list:new(), 5))).

next_clamps_at_the_end_test() ->
    S = sonde_list:select(sonde_list:new(), 4),
    ?assertEqual(4, sonde_list:selected(sonde_list:next(S, 5))).

prev_clamps_at_the_start_test() ->
    S = sonde_list:select(sonde_list:new(), 0),
    ?assertEqual(0, sonde_list:selected(sonde_list:prev(S, 5))).

navigation_on_empty_list_stays_unselected_test() ->
    ?assertEqual(none, sonde_list:selected(sonde_list:next(sonde_list:new(), 0))),
    ?assertEqual(none, sonde_list:selected(sonde_list:prev(sonde_list:new(), 0))).

navigation_clamps_a_stale_index_test() ->
    %% A selection left past the end of a now-shorter list still steps sanely.
    S = sonde_list:select(sonde_list:new(), 99),
    ?assertEqual(4, sonde_list:selected(sonde_list:next(S, 5))),
    ?assertEqual(3, sonde_list:selected(sonde_list:prev(S, 5))).

%%% -- reconciliation --------------------------------------------------

render_returns_reconciled_state_test() ->
    %% Viewport 3 rows, selection 5 -> offset pulled to 5 - 3 + 1 = 3.
    {_, State} = render(#{items => items(10)}, 10, 3, sonde_list:select(sonde_list:new(), 5)),
...
render_empty_list_clears_selection_test() ->
    {_, State} = render(#{items => []}, 10, 5, sonde_list:select(sonde_list:new(), 0)),
...
degenerate_area_still_reconciles_state_test() ->
    ...
    B0 = buf(10, 5),
    {B1, State} = sonde_list:render(#{items => items(10)}, rect(0, 0, 0, 5), B0, #list_state{
...
selected_row_is_highlighted_across_full_width_test() ->
    ...
    {B, _} = render(Cfg, 6, 3, sonde_list:select(sonde_list:new(), 1)),
...
unselected_rows_indent_under_the_symbol_gutter_test() ->
    ...
    {B, _} = render(Cfg, 6, 2, sonde_list:select(sonde_list:new(), 1)),
...
base_style_fills_unselected_rows_full_width_test() ->
    ...
    {B, _} = render(Cfg, 6, 2, sonde_list:select(sonde_list:new(), 0)),
...
unstyled_rows_preserve_parent_background_test() ->
    ...
    %% shows through.
    Parent = sonde_widget:fill(buf(6, 2), rect(0, 0, 6, 2), #{bg => 3}),
    {B, _} = sonde_list:render(
        #{items => [<<"a">>, <<"b">>]},
        rect(0, 0, 6, 2),
        Parent,
        sonde_list:select(sonde_list:new(), 0)
    ),
    ?assertMatch(#cell{bg = 3}, cell(B, 5, 0)),
    ?assertMatch(#cell{bg = 3}, cell(B, 5, 1)).

no_highlight_symbol_draws_items_at_the_left_test() ->
    {B, _} = render(#{items => [<<"aa">>, <<"bb">>]}, 6, 2, sonde_list:select(sonde_list:new(), 0)),

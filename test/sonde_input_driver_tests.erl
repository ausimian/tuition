-module(sonde_input_driver_tests).
...
poll_decodes_bytes_test() ->
    ...
    {ok, Events, _St} = sonde_input_driver:poll(Handle, sonde_input:new(), 1000),
...
poll_lone_esc_flushes_on_timeout_test() ->
    ...
    St0 = sonde_input:new(),
    {ok, E1, St1} = sonde_input_driver:poll(Handle, St0, 1000),
    ?assertEqual([], E1),
    ?assert(sonde_input:pending(St1)),
    {ok, E2, St2} = sonde_input_driver:poll(Handle, St1, 1000),
...
poll_esc_sequence_not_flushed_test() ->
    ...
    {ok, E1, St1} = sonde_input_driver:poll(Handle, St0, 1000),
    ?assertEqual([], E1),
    {ok, E2, _St2} = sonde_input_driver:poll(Handle, St1, 1000),
...
poll_utf8_partial_not_flushed_on_timeout_test() ->
    ...
    {ok, E3, _St3} = sonde_input_driver:poll(Handle, St2, 1000),
...
poll_alt_utf8_not_flushed_to_escape_test() ->
    ...
    {ok, E3, St3} = sonde_input_driver:poll(Handle, St2, 1000),
    {ok, E4, _St4} = sonde_input_driver:poll(Handle, St3, 1000),
...
poll_error_propagates_test() ->
    ...
    ?assertEqual(
        {error, eof},
        sonde_input_driver:poll(Handle, sonde_input:new(), 1000)
    ).

handle(Script) ->
    {sonde_fake_term, sonde_fake_term:start(Script)}.

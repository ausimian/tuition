%%% Test-only terminal backend for exercising sonde_caps:probe/2 without a tty.
%%% It records everything written (so a test can assert the probe emitted the
%%% right query sequences) and replays a scripted list of {@link sonde_term:read/2}
...
%%% write-failure path is testable too. Use as the handle `{sonde_probe_term, Pid}'.
-module(sonde_probe_term).

-export([start/1, start/2, write/2, read/2, sent/1, loop/3]).

%% Start a backend serving `Script' (a list of read results) whose writes
%% succeed. Returns the pid to wrap in a {sonde_probe_term, Pid} handle.
-spec start([term()]) -> pid().
start(Script) ->
    start(Script, ok).

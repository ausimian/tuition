...
render(Cfg, W, H, State) ->
    tuition_table:render(Cfg, rect(0, 0, W, H), buf(W, H), State).
...
%% Sort-indicator glyphs, mirrored from tuition_table's private defines.
-define(ASC, 16#25B2).
-define(DESC, 16#25BC).
...
navigation_delegates_to_the_list_test() ->
    ?assertEqual(0, tuition_table:selected(tuition_table:next(tuition_table:new(), 5))),
    ?assertEqual(4, tuition_table:selected(tuition_table:prev(tuition_table:new(), 5))),
    ?assertEqual(2, tuition_table:selected(tuition_table:select(tuition_table:new(), 2))).

%%% -- sorting (pure) --------------------------------------------------

toggle_sort_new_column_starts_ascending_test() ->
    ?assertEqual({2, asc}, tuition_table:toggle_sort(none, 2)),
    ?assertEqual({2, asc}, tuition_table:toggle_sort({1, desc}, 2)).

toggle_sort_same_column_flips_direction_test() ->
    ?assertEqual({1, desc}, tuition_table:toggle_sort({1, asc}, 1)),
    ?assertEqual({1, asc}, tuition_table:toggle_sort({1, desc}, 1)).

apply_sort_none_is_identity_test() ->
    Rows = [[<<"b">>], [<<"a">>]],
    ?assertEqual(Rows, tuition_table:apply_sort(Rows, none)).

apply_sort_orders_ascending_by_column_text_test() ->
    Rows = [[<<"1">>, <<"charlie">>], [<<"2">>, <<"alpha">>], [<<"3">>, <<"bravo">>]],
    Sorted = tuition_table:apply_sort(Rows, {1, asc}),
...
apply_sort_orders_descending_test() ->
    ...
    ?assertEqual(
        [[<<"charlie">>], [<<"bravo">>], [<<"alpha">>]],
        tuition_table:apply_sort(Rows, {0, desc})
    ).
...
header_fills_full_width_with_its_style_test() ->
    ...
    {B, _} = render(Cfg, 20, 5, tuition_table:new()),
...
header_present_even_with_no_rows_test() ->
    {B, _} = render(base_cfg([]), 20, 1, tuition_table:new()),
    ?assertEqual($P, ch(B, 2, 0)).
...
columns_are_positioned_by_their_constraints_test() ->
    Rows = [[<<"p0">>, <<"alpha">>]],
    {B, _} = render(base_cfg(Rows), 20, 5, tuition_table:new()),
...
right_aligned_cell_sits_at_the_column_right_edge_test() ->
    ...
    {B, _} = render(Cfg, 10, 3, tuition_table:new()),
...
center_aligned_cell_is_centred_in_the_column_test() ->
    ...
    Cfg = #{columns => Cols, rows => [[<<"abc">>]]},
    {B, _} = render(Cfg, 9, 3, tuition_table:new()),
...
selected_row_is_highlighted_across_full_width_test() ->
    ...
    {B, _} = render(Cfg, 20, 5, tuition_table:select(tuition_table:new(), 0)),
...
unselected_rows_have_no_symbol_in_the_gutter_test() ->
    {B, _} = render(
        base_cfg([[<<"p0">>, <<"nm">>]]), 20, 5, tuition_table:select(tuition_table:new(), none)
    ),
...
base_row_style_fills_full_width_test() ->
    ...
    {B, _} = render(Cfg, 20, 5, tuition_table:select(tuition_table:new(), none)),
...
unstyled_rows_preserve_parent_background_test() ->
    ...
    %% glyphs overwrite.
    Parent = tuition_widget:fill(buf(20, 5), rect(0, 0, 20, 5), #{bg => 3}),
    {B, _} = tuition_table:render(
        #{columns => cols(), rows => [[<<"p0">>, <<"nm">>]], highlight_symbol => <<"> ">>},
        rect(0, 0, 20, 5),
        Parent,
        tuition_table:select(tuition_table:new(), none)
    ),
...
render_returns_reconciled_state_test() ->
    ...
    {_, State} = render(
        #{columns => Cols, rows => Rows}, 10, 3, tuition_table:select(tuition_table:new(), 5)
    ),
...
no_columns_draws_nothing_but_reconciles_test() ->
    B0 = buf(20, 5),
    {B1, State} = tuition_table:render(
        #{columns => [], rows => [[<<"a">>], [<<"b">>]]},
        rect(0, 0, 20, 5),
        B0,
        tuition_table:select(tuition_table:new(), 9)
    ),
    ?assertEqual(B0, B1),
...
lazy_rows_render_like_eager_rows_test() ->
    ...
    St = tuition_table:select(tuition_table:new(), 5),
...
lazy_rows_scroll_extent_uses_item_count_test() ->
    ...
    {B, State} = render(
        ...
        10,
        3,
        tuition_table:select(tuition_table:new(), 50)
    ),
    ...
    {_, Clamped} = render(
        ...
        10,
        3,
        tuition_table:select(tuition_table:new(), 200)
    ),
...
column_spacing_widens_the_gap_between_columns_test() ->
    ...
    {B, _} = render(Cfg, 12, 3, tuition_table:new()),

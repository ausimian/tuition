...
%% Eighth-block glyphs, mirrored from sonde_gauge's private defines.
-define(FULL, 16#2588).
-define(HALF, 16#258C).
...
render(Cfg, W, H) ->
    sonde_gauge:render(Cfg, rect(0, 0, W, H), buf(W, H)).

%% The whole frame as the bytes the renderer would emit, for matching label text.
frame(Buf, W, H) ->
    iolist_to_binary(sonde_render:diff(buf(W, H), Buf)).
...
unstyled_gauge_leaves_the_track_transparent_test() ->
    ...
    Parent = sonde_widget:fill(buf(6, 1), rect(0, 0, 6, 1), #{bg => 4}),
    B = sonde_gauge:render(#{ratio => 0.0, label => none}, rect(0, 0, 6, 1), Parent),
...
degenerate_area_draws_nothing_test() ->
    B0 = buf(10, 3),
    ?assertEqual(B0, sonde_gauge:render(#{ratio => 0.5}, rect(0, 0, 0, 3), B0)),
    ?assertEqual(B0, sonde_gauge:render(#{ratio => 0.5}, rect(0, 0, 10, 0), B0)).

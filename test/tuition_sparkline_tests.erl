-module(tuition_sparkline_tests).
...
%% Vertical eighth-block glyphs, mirrored from tuition_sparkline's private defines.
-define(FULL, 16#2588).
-define(E1, 16#2581).
-define(E2, 16#2582).
-define(E3, 16#2583).
...
render(Cfg, W, H) ->
    tuition_sparkline:render(Cfg, rect(0, 0, W, H), buf(W, H)).
...
empty_data_draws_nothing_test() ->
    B0 = buf(5, 2),
    ?assertEqual(B0, tuition_sparkline:render(#{data => []}, rect(0, 0, 5, 2), B0)).
...
degenerate_area_draws_nothing_test() ->
    B0 = buf(10, 3),
    ?assertEqual(B0, tuition_sparkline:render(#{data => [1, 2, 3]}, rect(0, 0, 0, 3), B0)),
    ?assertEqual(B0, tuition_sparkline:render(#{data => [1, 2, 3]}, rect(0, 0, 10, 0), B0)).

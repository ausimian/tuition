...
acceptance_vertical_30_70_test() ->
    Area = tuition_layout:area({80, 24}),
    Rects = tuition_layout:split(vertical, [{percent, 30}, {percent, 70}], Area),
    ?assertEqual(
        [
...
area_test() ->
    ?assertEqual(#rect{x = 0, y = 0, w = 80, h = 24}, tuition_layout:area({80, 24})),
    ?assertEqual(#rect{x = 0, y = 0, w = 1, h = 1}, tuition_layout:area({1, 1})).
...
horizontal_30_70_test() ->
    Area = tuition_layout:area({80, 24}),
    Rects = tuition_layout:split(horizontal, [{percent, 30}, {percent, 70}], Area),
    ?assertEqual(
        [
...
inherits_parent_origin_test() ->
    ...
    Rects = tuition_layout:split(vertical, [{percent, 50}, {percent, 50}], Parent),
    ?assertEqual(
        [
...
fixed_test() ->
    ...
    ?assertEqual(
        ...
        tuition_layout:split(vertical, [{fixed, 3}, fill], Area)
    ).
...
single_fill_takes_all_test() ->
    ...
    ?assertEqual(
        ...
        tuition_layout:split(vertical, [fill], Area)
    ).

equal_fills_split_evenly_test() ->
    Rects = tuition_layout:split(horizontal, [fill, fill], tuition_layout:area({80, 24})),
    ?assertEqual([40, 40], [R#rect.w || R <- Rects]).

weighted_fill_test() ->
    %% Weights 1:3 over 80 columns -> 20 and 60.
    Rects = tuition_layout:split(
        horizontal, [{fill, 1}, {fill, 3}], tuition_layout:area({80, 24})
    ),
...
fill_absorbs_remainder_after_fixed_and_percent_test() ->
    ...
    Rects = tuition_layout:split(
        horizontal, [{fixed, 10}, {percent, 25}, fill], tuition_layout:area({100, 24})
    ),
...
halves_of_odd_extent_tile_test() ->
    Area = tuition_layout:area({25, 24}),
    Rects = tuition_layout:split(horizontal, [{percent, 50}, {percent, 50}], Area),
...
thirds_via_fill_tile_test() ->
    Area = tuition_layout:area({100, 24}),
    Rects = tuition_layout:split(horizontal, [fill, fill, fill], Area),
...
under_subscription_leaves_gap_test() ->
    Area = tuition_layout:area({80, 24}),
    Rects = tuition_layout:split(vertical, [{percent, 25}, {percent, 25}], Area),
...
over_subscription_fixed_clamps_test() ->
    Area = tuition_layout:area({80, 10}),
    Rects = tuition_layout:split(vertical, [{fixed, 8}, {fixed, 8}, {fixed, 8}], Area),
...
over_subscription_percent_clamps_test() ->
    Area = tuition_layout:area({100, 24}),
    Rects = tuition_layout:split(horizontal, [{percent, 80}, {percent, 80}], Area),
...
empty_constraints_test() ->
    ?assertEqual([], tuition_layout:split(vertical, [], tuition_layout:area({80, 24}))).

zero_extent_axis_test() ->
    %% A one-row area split vertically leaves the second pane no rows.
    Rects = tuition_layout:split(vertical, [fill, fill], #rect{x = 0, y = 0, w = 80, h = 1}),
...
nested_layout_test() ->
    Area = tuition_layout:area({80, 24}),
    [Header, Body, Footer] =
        tuition_layout:split(vertical, [{fixed, 1}, fill, {fixed, 1}], Area),
    ?assertEqual(#rect{x = 0, y = 0, w = 80, h = 1}, Header),
    ?assertEqual(#rect{x = 0, y = 1, w = 80, h = 22}, Body),
    ?assertEqual(#rect{x = 0, y = 23, w = 80, h = 1}, Footer),
    [Left, Right] = tuition_layout:split(horizontal, [{percent, 50}, {percent, 50}], Body),

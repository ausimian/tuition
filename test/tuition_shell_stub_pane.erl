%%% Shared logic for the shell's test stub panes ({@link tuition_shell_pane_a} /
%%% {@link tuition_shell_pane_b}). Not a pane itself — the two thin pane modules
...
%%% refresh.
-module(tuition_shell_stub_pane).
...
render(#rect{x = X, y = Y, h = H}, Buf, #st{tag = Tag, mode = Mode, filter = Filter} = St) ->
    ...
    Style = #{fg => 7},
    Buf1 = tuition_render:put_text(Buf, X, Y, <<Tag/binary, " body">>, Style),
    Buf2 =
        case Mode =:= filter andalso H > 1 of
            true -> tuition_render:put_text(Buf1, X, Y + 1, <<"filter: ", Filter/binary>>, Style);
            false -> Buf1
        end,
    {Buf2, St}.

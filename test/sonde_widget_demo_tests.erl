-module(sonde_widget_demo_tests).

-include_lib("eunit/include/eunit.hrl").

%% Ascending sort indicator, mirrored from sonde_table's private define.
-define(ASC, 16#25B2).
...
apply(Events) ->
    sonde_widget_demo:apply_events(Events, sonde_widget_demo:new()).

%%% -- selection folding (pure) ----------------------------------------

down_moves_selection_forward_test() ->
    {ok, S} = apply([key(down), key(down)]),
    ?assertEqual(2, sonde_widget_demo:selection(S)).

up_from_the_top_stays_put_test() ->
    {ok, S} = apply([key(up)]),
    ?assertEqual(0, sonde_widget_demo:selection(S)).

j_and_k_navigate_like_down_and_up_test() ->
    {ok, S} = apply([char($j), char($j), char($k)]),
    ?assertEqual(1, sonde_widget_demo:selection(S)).

home_jumps_to_the_first_row_test() ->
    {ok, S0} = apply([key(down), key(down), key(down)]),
    {ok, S1} = sonde_widget_demo:apply_events([key(home)], S0),
    ?assertEqual(0, sonde_widget_demo:selection(S1)).

end_jumps_to_the_last_row_and_is_stable_test() ->
    {ok, S1} = apply([key('end')]),
    {ok, S2} = sonde_widget_demo:apply_events([key(down)], S1),
    ?assert(sonde_widget_demo:selection(S1) > 0),
    ?assertEqual(sonde_widget_demo:selection(S1), sonde_widget_demo:selection(S2)).
...
ctrl_c_quits_test() ->
    ?assertEqual(
        quit, sonde_widget_demo:apply_events([{key, {ctrl, $c}, []}], sonde_widget_demo:new())
    ).
...
build_frame_composes_the_widgets_test() ->
    {Buf, _} = sonde_widget_demo:build_frame({40, 12}, sonde_widget_demo:new()),
    B = iolist_to_binary(sonde_render:diff(sonde_render:new({40, 12}), Buf)),
...
build_frame_scrolls_to_keep_selection_visible_test() ->
    ...
    {ok, State} = sonde_widget_demo:apply_events(
        lists:duplicate(28, key(down)), sonde_widget_demo:new()
    ),
    {Buf, _} = sonde_widget_demo:build_frame({80, 24}, State),
    B = iolist_to_binary(sonde_render:diff(sonde_render:new({80, 24}), Buf)),
...
pressing_s_sorts_by_name_and_marks_the_header_test() ->
    ...
    {Buf, _} = sonde_widget_demo:build_frame({60, 20}, State),
    B = iolist_to_binary(sonde_render:diff(sonde_render:new({60, 20}), Buf)),
...
run_loop(Script, Size) ->
    ...
    ?assertEqual(ok, sonde_widget_demo:start(Opts)),
    drain(<<>>, false).

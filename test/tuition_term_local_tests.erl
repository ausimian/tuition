-module(tuition_term_local_tests).
...
open_without_tty_errors_test() ->
    ?assertMatch({error, _}, tuition_term_local:open(#{})).
...
saved_opts_cooperative_captures_echo_binary_encoding_test() ->
    ...
    try
        ?assertEqual(
            ...
            tuition_term_local:saved_opts(Io, [{echo, false}, binary, {encoding, latin1}])
        )
    after
        tuition_fake_io:stop(Io)
    end.
...
saved_opts_noshell_omits_echo_test() ->
    ...
    try
        ?assertEqual(
            ...
            tuition_term_local:saved_opts(Io, [binary, {encoding, latin1}])
        )
    after
        tuition_fake_io:stop(Io)
    end.
...
saved_opts_defaults_when_absent_test() ->
    Io = tuition_fake_io:start(#{getopts => []}),
    try
        ?assertEqual(
...
reader_forwards_device_reads_test() ->
    Io = tuition_fake_io:start(#{chars => [<<"a">>, "b", <<27>>]}),
    Owner = self(),
    Reader = spawn(tuition_term_local, reader_loop, [Owner, Io]),
    try
...
reader_forwards_error_and_stops_test() ->
    Io = tuition_fake_io:start(#{chars => [{error, enotsup}]}),
    Owner = self(),
    Reader = spawn(tuition_term_local, reader_loop, [Owner, Io]),
    try
...
cooperative_restore_writes_seq_and_restores_opts_test() ->
    ...
    try
        ?assertEqual(ok, tuition_term_local:do_restore(Io, Seq, Prev, false)),
        #{puts := Puts, setopts := SetOpts} = tuition_fake_io:recorded(Io),
...
recv_data(Reader) ->
    receive
        {tuition_term_local, Reader, {data, Bytes}} -> Bytes
    after 1000 -> error(no_data)
    end.

recv_end(Reader) ->
    receive
        {tuition_term_local, Reader, eof} -> eof;
        {tuition_term_local, Reader, {error, Reason}} -> {error, Reason}

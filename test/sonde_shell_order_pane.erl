%%% Test-only pane that logs its {@link sonde_pane:setup/0} / {@link
%%% sonde_pane:teardown/1} calls, so a test can assert the shell acquires and
...
setup() ->
    N =
        case get(sonde_pane_lifecycle_seq) of
            undefined -> 1;
            Seq -> Seq + 1
        end,
    put(sonde_pane_lifecycle_seq, N),
    log({setup, N}),
    N.
...
log(Event) ->
    put(sonde_pane_lifecycle, [Event | current()]).

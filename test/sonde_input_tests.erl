...
sgr_mouse_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e[<0;10">>, St0),
    {E2, _St2} = sonde_input:parse(<<";20M">>, St1),
    ?assertEqual([], E1),
    ?assert(sonde_input:pending(St1)),
...
bracketed_paste_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e[200~he">>, St0),
    {E2, St2} = sonde_input:parse(<<"llo">>, St1),
    {E3, St3} = sonde_input:parse(<<"\e[201~">>, St2),
    ?assertEqual([], E1),
    ?assertEqual([], E2),
    ?assert(sonde_input:pending(St1)),
    ?assert(sonde_input:pending(St2)),
    %% A paste in progress must not force the short ESC timeout (it would truncate
    %% a slow paste), even though bytes are buffered.
    ?assertNot(sonde_input:awaiting_escape(St2)),
    ?assertEqual([{paste, <<"hello">>}], E3),
    ?assertNot(sonde_input:pending(St3)).
...
bracketed_paste_terminator_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e[200~hi\e[20">>, St0),
    {E2, _St2} = sonde_input:parse(<<"1~">>, St1),
    ?assertEqual([], E1),
...
bracketed_paste_flush_emits_partial_test() ->
    ...
    {[], St} = sonde_input:parse(<<"\e[200~ab">>, sonde_input:new()),
    ?assert(sonde_input:pending(St)),
...
alt_multibyte_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e">>, St0),
    {E2, St2} = sonde_input:parse(<<16#C3>>, St1),
    {E3, _St3} = sonde_input:parse(<<16#A9>>, St2),
    ?assertEqual([], E1),
    ?assertEqual([], E2),
    ?assert(sonde_input:pending(St2)),
...
double_escape_buffers_then_flushes_to_two_escapes_test() ->
    ...
    {Events, St} = sonde_input:parse(<<"\e\e">>, sonde_input:new()),
    ?assertEqual([], Events),
    ?assert(sonde_input:pending(St)),
    ?assert(sonde_input:awaiting_escape(St)),
...
alt_prefixed_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e">>, St0),
    {E2, St2} = sonde_input:parse(<<"\e">>, St1),
    {E3, St3} = sonde_input:parse(<<"[">>, St2),
    {E4, St4} = sonde_input:parse(<<"A">>, St3),
    ?assertEqual([], E1 ++ E2 ++ E3),
    ?assert(sonde_input:awaiting_escape(St1)),
    ?assert(sonde_input:awaiting_escape(St2)),
    ?assert(sonde_input:awaiting_escape(St3)),
    ?assertEqual([{key, up, [alt]}], E4),
    ?assertNot(sonde_input:pending(St4)).
...
alt_prefixed_incomplete_flushes_to_escapes_test() ->
    ...
    {[], St} = sonde_input:parse(<<"\e\e[">>, sonde_input:new()),
    ?assert(sonde_input:awaiting_escape(St)),
    ?assertEqual(
...
triple_escape_flushes_to_three_escapes_test() ->
    ...
    %% two more Escapes.
    {Events, St} = sonde_input:parse(<<"\e\e\e">>, sonde_input:new()),
...
flush_keeps_trailing_utf8_partial_buffered_test() ->
    ...
    {[], St0} = sonde_input:parse(<<"\e[", 16#C3>>, sonde_input:new()),
    ?assert(sonde_input:awaiting_escape(St0)),
    {Flushed, St1} = sonde_input:flush(St0),
    ?assertEqual([{key, esc, []}, {key, {char, $[}, []}], Flushed),
    ?assert(sonde_input:pending(St1)),
    ?assertNot(sonde_input:awaiting_escape(St1)),
    %% ...so the continuation byte still completes é (U+00E9), not a replacement.
    {Events, _St2} = sonde_input:parse(<<16#A9>>, St1),
...
bad_continuation_does_not_swallow_next_key_test() ->
    ...
    {[], St} = sonde_input:parse(<<16#E0, 16#A0>>, sonde_input:new()),
    ?assert(sonde_input:pending(St)),
    ?assertNot(sonde_input:awaiting_escape(St)),
    %% ...and completes when the final byte arrives (U+0800).
    {Events, _} = sonde_input:parse(<<16#80>>, St),
...
csi_split_across_reads_test() ->
    ...
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<"\e">>, St0),
    {E2, St2} = sonde_input:parse(<<"[">>, St1),
    {E3, St3} = sonde_input:parse(<<"A">>, St2),
    ?assertEqual([], E1),
    ?assertEqual([], E2),
...
utf8_split_across_reads_test() ->
    St0 = sonde_input:new(),
    {E1, St1} = sonde_input:parse(<<16#C3>>, St0),
    ?assertEqual([], E1),
    ?assert(sonde_input:pending(St1)),
    {E2, _St2} = sonde_input:parse(<<16#A9>>, St1),
...
esc_then_printable_is_alt_across_reads_test() ->
    ...
    ?assertEqual([], E1),
    ?assert(sonde_input:pending(St1)),
    {E2, _St2} = sonde_input:parse(<<"a">>, St1),
    ?assertEqual([{key, {char, $a}, [alt]}], E2).

%%% -- lone-ESC disambiguation via flush -------------------------------

lone_esc_buffers_then_flushes_to_escape_test() ->
    {Events, St} = sonde_input:parse(<<"\e">>, sonde_input:new()),
    ?assertEqual([], Events),
    ?assert(sonde_input:pending(St)),
    {Flushed, St2} = sonde_input:flush(St),
    ?assertEqual([{key, esc, []}], Flushed),
    ?assertNot(sonde_input:pending(St2)).

flush_on_empty_is_noop_test() ->
    ?assertEqual([], flush_events(sonde_input:new())).

awaiting_escape_distinguishes_esc_from_utf8_partial_test() ->
    ?assertNot(sonde_input:awaiting_escape(sonde_input:new())),
    %% Lone ESC and partial escape sequences are ESC-ambiguous.
    {[], EscSt} = sonde_input:parse(<<"\e">>, sonde_input:new()),
    ?assert(sonde_input:awaiting_escape(EscSt)),
    {[], CsiSt} = sonde_input:parse(<<"\e[">>, sonde_input:new()),
    ?assert(sonde_input:awaiting_escape(CsiSt)),
    %% A split UTF-8 lead byte is pending but NOT ESC-ambiguous.
    {[], Utf8St} = sonde_input:parse(<<16#C3>>, sonde_input:new()),
    ?assert(sonde_input:pending(Utf8St)),
    ?assertNot(sonde_input:awaiting_escape(Utf8St)),
    ...
    {[], AltUtf8St} = sonde_input:parse(<<"\e", 16#C3>>, sonde_input:new()),
    ?assert(sonde_input:pending(AltUtf8St)),
    ?assertNot(sonde_input:awaiting_escape(AltUtf8St)).

flush_truncated_utf8_yields_replacement_test() ->
    {[], St} = sonde_input:parse(<<16#C3>>, sonde_input:new()),
...
flush_incomplete_csi_falls_back_to_escape_test() ->
    ...
    {[], St} = sonde_input:parse(<<"\e[">>, sonde_input:new()),
    ?assertEqual(
        [{key, esc, []}, {key, {char, $[}, []}],
        flush_events(St)
    ).
...
decode(Bytes) ->
    {Events, _St} = sonde_input:parse(Bytes, sonde_input:new()),
    Events.

flush_events(St) ->
    {Events, _St} = sonde_input:flush(St),
    Events.

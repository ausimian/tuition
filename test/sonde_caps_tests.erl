...
parse_full_reply_enables_all_test() ->
    ...
    ?assertEqual(
        ...
        sonde_caps:parse_replies(Buf)
    ).

parse_truecolor_semicolon_form_test() ->
    Caps = sonde_caps:parse_replies(<<(?TRUECOLOR_SEMI)/binary, (?DA)/binary>>),
...
parse_non_truecolor_sgr_readback_test() ->
    Caps = sonde_caps:parse_replies(<<"\eP1$r0;38;5;9m\e\\", (?DA)/binary>>),
    ?assertNot(Caps#caps.truecolor).

%% An invalid DECRQSS reply (`0 $ r') means the query was not understood.
parse_invalid_decrqss_is_off_test() ->
    Caps = sonde_caps:parse_replies(<<"\eP0$r\e\\", (?DA)/binary>>),
    ?assertNot(Caps#caps.truecolor).
...
parse_misparsed_sgr_tail_is_not_truecolor_test() ->
    Caps = sonde_caps:parse_replies(<<"\eP1$r0;1;2;3m\e\\", (?DA)/binary>>),
    ?assertNot(Caps#caps.truecolor).
...
parse_partial_reply_leaves_rest_off_test() ->
    Caps = sonde_caps:parse_replies(<<(?SYNC_OK)/binary, (?DA)/binary>>),
...
parse_decrqm_zero_is_unsupported_test() ->
    Caps = sonde_caps:parse_replies(<<"\e[?2026;0$y", (?DA)/binary>>),
    ?assertNot(Caps#caps.sync_output).
...
parse_decrqm_permanently_reset_is_unsupported_test() ->
    Caps = sonde_caps:parse_replies(<<"\e[?2026;4$y", (?DA)/binary>>),
    ?assertNot(Caps#caps.sync_output).
...
parse_decrqm_permanently_set_is_supported_test() ->
    Caps = sonde_caps:parse_replies(<<"\e[?2026;3$y", (?DA)/binary>>),
...
parse_ignores_sentinel_and_noise_test() ->
    ?assertEqual(#caps{}, sonde_caps:parse_replies(?DA)),
    ?assertEqual(#caps{}, sonde_caps:parse_replies(<<"garbage", (?DA)/binary>>)),
    ?assertEqual(#caps{}, sonde_caps:parse_replies(<<>>)).

%% A truncated control string at the tail is dropped cleanly, not looped on.
parse_truncated_dcs_is_dropped_test() ->
    ?assertEqual(#caps{}, sonde_caps:parse_replies(<<"\eP1$r0;38;2;1;2;3">>)).
...
decode_replies_pure_replies_leave_no_residue_test() ->
    {Caps, Residue} = sonde_caps:decode_replies(<<(?SYNC_OK)/binary, (?DA)/binary>>),
    ?assertEqual(#caps{sync_output = true}, Caps),
    ?assertEqual(<<>>, Residue).
...
decode_replies_trailing_key_is_residue_test() ->
    {Caps, Residue} = sonde_caps:decode_replies(<<"\e[?62;c", "q">>),
    ?assertEqual(#caps{}, Caps),
...
decode_replies_trailing_incomplete_escape_is_residue_test() ->
    {Caps, Residue} = sonde_caps:decode_replies(<<"\e[?62;c", "\e[">>),
    ?assertEqual(#caps{}, Caps),
...
probe_writes_queries_and_decodes_test() ->
    Buf = <<
        ...
        (?DA)/binary
    >>,
    Pid = sonde_probe_term:start([{ok, Buf}]),
    {Caps, _Residue} = sonde_caps:probe({sonde_probe_term, Pid}, 100),
    ?assertEqual(
        #caps{
            truecolor = true,
            sync_output = true,
    ...
    Sent = sonde_probe_term:sent(Pid),
    [
...
probe_accumulates_split_reads_test() ->
    ...
    Pid = sonde_probe_term:start([{ok, A}, {ok, B}]),
    {Caps, _Residue} = sonde_caps:probe({sonde_probe_term, Pid}, 100),
    ?assert(Caps#caps.truecolor),
    ?assert(Caps#caps.sync_output).
...
probe_only_sentinel_is_baseline_test() ->
    Pid = sonde_probe_term:start([{ok, ?DA}]),
    %% The DA1 sentinel is a complete reply, so it is consumed: no residue.
    ?assertEqual({#caps{}, <<>>}, sonde_caps:probe({sonde_probe_term, Pid}, 100)).
...
probe_silent_terminal_times_out_to_baseline_test() ->
    Pid = sonde_probe_term:start([]),
    ?assertEqual({#caps{}, <<>>}, sonde_caps:probe({sonde_probe_term, Pid}, 50)).

%% A write failure short-circuits to the baseline (no reads attempted).
probe_write_error_is_baseline_test() ->
    Pid = sonde_probe_term:start([{ok, ?DA}], {error, closed}),
    ?assertEqual({#caps{}, <<>>}, sonde_caps:probe({sonde_probe_term, Pid}, 100)).
...
probe_modified_f3_does_not_end_probe_test() ->
    Replies = <<
        ...
        (?DA)/binary
    >>,
    Pid = sonde_probe_term:start([{ok, <<"\e[1;2R">>}, {ok, Replies}]),
    {Caps, _Residue} = sonde_caps:probe({sonde_probe_term, Pid}, 100),
    ?assertEqual(
        #caps{
            truecolor = true,
...
apply_colorterm_enables_truecolor_test() ->
    ?assert((sonde_caps:apply_colorterm("truecolor", #caps{}))#caps.truecolor),
    ?assert((sonde_caps:apply_colorterm("24bit", #caps{}))#caps.truecolor).
...
apply_colorterm_other_values_are_noops_test() ->
    ?assertEqual(#caps{}, sonde_caps:apply_colorterm(false, #caps{})),
    ?assertEqual(#caps{}, sonde_caps:apply_colorterm("256color", #caps{})),
    ?assert((sonde_caps:apply_colorterm(false, #caps{truecolor = true}))#caps.truecolor).
...
apply_colorterm_preserves_other_caps_test() ->
    ...
    ?assertEqual(
        ...
        sonde_caps:apply_colorterm("truecolor", Caps)
    ).

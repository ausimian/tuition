...
term_seam_dispatch_test() ->
    ?assertMatch({error, _}, sonde_term:open(sonde_term_local, #{})).
...
start_open_error_propagates_test() ->
    ?assertEqual(
        {error, no_tty},
        sonde_tui_demo:start(#{backend => sonde_loop_term, open => {error, no_tty}})
    ).
...
run_loop(ProbeReply, Script, Size) ->
    ...
    ?assertEqual(ok, sonde_tui_demo:start(Opts)),
    drain(<<>>, false).

...
ascii_test() ->
    ?assertEqual(1, sonde_width:width($a)),
    ?assertEqual(1, sonde_width:width($Z)),
    ?assertEqual(1, sonde_width:width("~")),
    %% A space still occupies a column.
    ?assertEqual(1, sonde_width:width($\s)),
    ?assertEqual(1, sonde_width:width(<<" ">>)).

wide_test() ->
    %% CJK ideograph 日 U+65E5 and fullwidth Latin Ａ U+FF21 are double-width.
    ?assertEqual(2, sonde_width:width(16#65E5)),
    ?assertEqual(2, sonde_width:width(<<"日"/utf8>>)),
    ?assertEqual(2, sonde_width:width(16#FF21)),
    %% Hangul syllable 가 U+AC00 and Hiragana あ U+3042.
    ?assertEqual(2, sonde_width:width(16#AC00)),
    ?assertEqual(2, sonde_width:width(16#3042)).

zero_width_test() ->
    %% Combining acute accent (Mn), ZWSP (Cf) and a C0 control (Cc).
    ?assertEqual(0, sonde_width:width(16#0301)),
    ?assertEqual(0, sonde_width:width(16#200B)),
    ?assertEqual(0, sonde_width:width(16#0001)),
    %% DEL and a lone ZWJ / variation selector are also zero on their own.
    ?assertEqual(0, sonde_width:width(16#007F)),
    ?assertEqual(0, sonde_width:width(16#200D)),
    ?assertEqual(0, sonde_width:width(16#FE0F)).

combining_cluster_test() ->
    %% "é" spelled as e + combining acute is one cluster, width 1.
    ?assertEqual(1, sonde_width:width([$e, 16#0301])),
    ?assertEqual(1, sonde_width:width(<<$e, "́"/utf8>>)).
...
emoji_presentation_test() ->
    ...
    ?assertEqual(1, sonde_width:width(16#2764)),
    ?assertEqual(2, sonde_width:width([16#2764, 16#FE0F])),
    %% Keycap sequence "1️⃣" = '1' + VS16 + U+20E3 renders double-width.
    ?assertEqual(2, sonde_width:width([$1, 16#FE0F, 16#20E3])),
    %% Unqualified keycap (no VS16): '1' + U+20E3 still renders as a 2-col
    %% keycap emoji, and "1⃣x" is 2 + 1 = 3 columns.
    ?assertEqual(2, sonde_width:width([$1, 16#20E3])),
    ?assertEqual(3, sonde_width:swidth([$1, 16#20E3, $x])).

zwj_sequence_test() ->
    %% Family 👨‍👩‍👧 = MAN ZWJ WOMAN ZWJ GIRL is a single cluster, width 2.
    Family = [16#1F468, 16#200D, 16#1F469, 16#200D, 16#1F467],
    ?assertEqual(2, sonde_width:width(Family)),
    ?assertEqual(2, sonde_width:width(<<"👨‍👩‍👧"/utf8>>)).
...
flag_emoji_test() ->
    ...
    ?assertEqual(2, sonde_width:width(US)),
    ?assertEqual(2, sonde_width:width(<<"🇺🇸"/utf8>>)),
    %% A lone regional indicator also renders wide.
    ?assertEqual(2, sonde_width:width(16#1F1FA)).
...
emoji_modifier_test() ->
    ...
    ?assertEqual(1, sonde_width:width(16#261D)),
    ?assertEqual(2, sonde_width:width([16#261D, 16#1F3FD])),
    ?assertEqual(2, sonde_width:width(<<"☝🏽"/utf8>>)),
    ...
    ?assertEqual(3, sonde_width:width([$a, 16#1F3FD])),
    ?assertEqual(3, sonde_width:swidth([$a, 16#1F3FD])).
...
indic_zwj_test() ->
    ...
    ?assertEqual(1, sonde_width:width([16#0915, 16#094D, 16#200D])),
    ...
    ?assertEqual(2, sonde_width:swidth([16#0915, 16#094D, 16#200D, 16#0937])).
...
malformed_emoji_base_test() ->
    ...
    ?assertEqual(1, sonde_width:width([$a, 16#FE0F])),
    ?assertEqual(1, sonde_width:swidth([$a, 16#FE0F])),
    ...
    ?assertEqual(4, sonde_width:width([16#65E5, 16#1F3FD])),
    ?assertEqual(4, sonde_width:swidth([16#65E5, 16#1F3FD])).

empty_test() ->
    ?assertEqual(0, sonde_width:width(<<>>)),
    ?assertEqual(0, sonde_width:width("")).

%%% -- swidth/1: whole strings -----------------------------------------

swidth_ascii_test() ->
    ?assertEqual(0, sonde_width:swidth("")),
    ?assertEqual(5, sonde_width:swidth("hello")),
    ?assertEqual(11, sonde_width:swidth(<<"hello world">>)).

swidth_mixed_test() ->
    %% "aあb" = 1 + 2 + 1 = 4, via both a binary and a codepoint list.
    ?assertEqual(4, sonde_width:swidth(<<"aあb"/utf8>>)),
    ?assertEqual(4, sonde_width:swidth([$a, 16#3042, $b])).

swidth_combining_test() ->
    %% Combining marks add no columns: "e" + acute + "x" is 2 columns.
    ?assertEqual(2, sonde_width:swidth([$e, 16#0301, $x])),
    %% A ZWJ family emoji contributes a single double-width cluster.
    ?assertEqual(2, sonde_width:swidth(<<"👨‍👩‍👧"/utf8>>)),
    ?assertEqual(6, sonde_width:swidth(<<"[👨‍👩‍👧]!!"/utf8>>)).

swidth_bad_utf8_test() ->
    %% Best-effort: a stray, undecodable byte counts as one column; no crash.
    %% "a","b",<0xFF>,"c","d" => 1+1+1+1+1 = 5.
    ?assertEqual(5, sonde_width:swidth(<<"ab", 16#FF, "cd">>)),
...
swidth_split_utf8_test() ->
    ...
    ?assertEqual(2, sonde_width:swidth([<<16#E3>>, <<16#81, 16#82>>])),
    ?assertEqual(2, sonde_width:swidth([<<16#E3, 16#81>>, <<16#82>>])),
    %% Boundary between two characters: "aあ" split as "a"+<<E3 81>> | <<82>>.
    ?assertEqual(3, sonde_width:swidth([<<$a, 16#E3, 16#81>>, <<16#82>>])).
...
oracle_disagreement_direction_test() ->
    ?assertEqual(2, sonde_width:width(16#4DC0)),
    ?assertEqual(2, sonde_width:width(16#1D300)),
    %% U+0903 DEVANAGARI SIGN VISARGA is a spacing (Mc) mark: width 1 here.
    ?assertEqual(1, sonde_width:width(16#0903)),
    %% U+0378 is unassigned: we reserve a single column.
    ?assertEqual(1, sonde_width:width(16#0378)).

defmodule Tuition.MixProject do
  ...
  # The `guides/*.md` extras are the narrative on-ramp (issue #55) — grouped
  # under "Guides" — and the `notebooks/*.livemd` extras are the runnable
  # per-widget showcases (issue #56): ExDoc renders each as a docs page, copies
  # the raw `.livemd` next to it, and adds the Run-in-Livebook badge itself —
  # pointed at that copy, resolved from the page's own URL, so a notebook carries
  # no badge markup of its own. Kept in step with the `ex_doc` stanza in
  # `rebar.config` — the two build systems each carry their own copy of this list.
  defp docs do
    [
      main: "readme",
      extras: [
